...
class CleanedStream(Stream):
    ...
    def tag(self, the_tag, **kwargs):
        ...
        Args:
            the_tag (str): tag to apply
        '"""
        return baseutils.CleanSequence.tag(self, the_tag, **kwargs)
    ...
    def select(self, **kwargs):
        ...
        if len(out_stream) == 0:
            ...
            tagged = Stream(baseutils.CleanSequence.seedid_tag(self))
            out_stream =  tagged.select(**kwargs)
            out_stream = baseutils.CleanSequence.seedid_untag(out_stream)
        if len(out_stream) == 0:
