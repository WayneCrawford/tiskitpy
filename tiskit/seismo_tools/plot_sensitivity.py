...
def _plot_sensitivity(channel,seed_id,args):
    ...
    if args.dyn_is_min_counts:
        ...
        print(f'minimum A/D counts provided:  {min_counts:d}')
        print(f'A/D Dynamic range calculated: {args.dyn_range:.0f}dB')
    else:
        min_counts=args.max_counts/np.power(10,args.dyn_range/20)
        print(f'A/D dynamic range provided:  {args.dyn_range:.0f}dB')
        print(f'min_counts calculated:       {min_counts:.1f}')
    ...
    if args.gain_mult:
        r *= args.gain_mult
    plt.semilogx(f[1:]**-1,20*np.log10(min_counts*np.reciprocal(abs(r[1:]))),
                label='Min sensitivity')
    plt.semilogx(f[1:]**-1,20*np.log10(args.max_counts*np.reciprocal(abs(r[1:]))),
                label='Max sensitivity')

    # Plot clip levels, if specified
    if args.clip_vel:
        lev=args.clip_vel*(2*np.pi*f)
        plt.semilogx(f[1:]**-1,20*np.log10(lev[1:]),':',label='sensor clip level')
    if args.clip_disp:
