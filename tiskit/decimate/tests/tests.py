...
class TestMethods(unittest.TestCase):
    ...
    def test_decimate_impulse(self):
        ...
        for d in range(2,8):
            ...
            tr2 = decim.run(tr)
            if plot:
                plot_compare(tr, tr2, d, '2016-12-12T20:54:27', '2016-12-12T20:54:33',
    ...
    def test_modify_stationxml(self):
        ...
        inv_SIO = inv.select(station='S1*D')  # Restrict to Scripps to shorten prints
        ...
        #self.assertEqual(inv_SIO, read_inventory(str(self.test_path / 
...
def plot_compare(tr1, tr2, decim, startdate, enddate, show=False,
    ...
    :type startdate, enddate: str in datetime isoformat
    """
    # print(f'saving to {savefig}')
    plt.plot_date(tr1.times(type='matplotlib'), tr1.data, 'r-', label='original')
    plt.plot(tr2.times(type='matplotlib'), tr2.data, 'b-', label=f'decim{decim}')
    plt.xlim(datetime.datetime.fromisoformat(startdate), datetime.datetime.fromisoformat(enddate))
    plt.legend()
    if show:
        plt.show()
