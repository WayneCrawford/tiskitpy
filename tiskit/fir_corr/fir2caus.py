...
def fir2caus(stream,firCorrFileName, FIRdecim):
    ...
    for tr in s:
        ...
        tr.data=np.array(data_corr,dtype=data_type)   # Force back to original data type:
...
def  _ipol2(xin, debug=False) :
    ...
    g2_1 = np.array([ -0.0002616,  0.0009302, -0.0023258,  0.0049142, -0.0092537,
                       0.0161355, -0.0266150,  0.0424554, -0.0670612,  0.1091686,
                      -0.2008408,  0.6327541,  0.6327542, -0.2008408,  0.1091686,
                      -0.0670612,  0.0424554, -0.0266150,  0.0161355, -0.0092537,
                       0.0049142, -0.0023258,  0.0009302, -0.0002616],
                    dtype='double')
...
def  _ipol3(xin) :
    ...
    g3_1 = np.array([ -0.0002324,  0.0008256, -0.0020654,  0.0043684, -0.0082410,  
                       0.0144087, -0.0238643,  0.0383080, -0.0611438,  0.1015046, 
                      -0.1959616,  0.8226883,  0.4111037, -0.1564917,  0.0885536, 
                      -0.0553522,  0.0353777, -0.0223077,  0.0135759, -0.0078056,  
                       0.0041522, -0.0019670,  0.0007871, -0.0002211],
                    dtype='double')
    
    # xx1 = interpolated sample 1/3 between index i and i+1
    # xx2 = interpolated sample 2/3 between index i and i+1 : use g3_1[] in reverse
...
def  __ipol5(xin) :
    ...
    g5_1 = np.array([ -0.0001611,  0.0005720, -0.0014316,  0.0030307, -0.0057263,
                       0.0100352, -0.0166788,  0.0269186, -0.0433567,  0.0732492,
                      -0.1480766,  0.9320452,  0.2327664, -0.0984035,  0.0572469,
                      -0.0362360,  0.0233232, -0.0147709,  0.0090151, -0.0051932,
                       0.0027660, -0.0013112,  0.0005248, -0.0001473],
                    dtype='double')
    g5_2 = np.array([ -0.0002526,  0.0008977, -0.0022452,  0.0047467, -0.0089479,
                       0.0156272, -0.0258392,  0.0413715, -0.0657512,  0.1082703,
                      -0.2048060,  0.7525200,  0.5015040, -0.1790148,  0.0997642,
                      -0.0619420,  0.0394431, -0.0248145,  0.0150789, -0.0086611,
                       0.0046043, -0.0021804,  0.0008723, -0.0002452],
                    dtype='double')
    
    # xx1 = interpolated sample 1/5 between index i and i+1 : use g5_1
    # xx2 = interpolated sample 2/5 between index i and i+1 : use g5_2
    # xx3 = interpolated sample 2/5 between index i and i+1 : use g5_2 in reverse
    # xx4 = interpolated sample 2/5 between index i and i+1 : use g5_1 in reverse
...
def fir2caus_subr(intrace,firCorrFileName,returnOrigDType=True) :
    '''
    Implements FIR filter correction described in chapter 8 of "Of poles and zeros"
    
    Inputs:
        intrace=numpy 1d float array
        firCorrFileName = string containing JSON parameter file name
        returnOrigDtype = return output trace as same type as input trace (True, default)
                            or as float double (False)
    
    Outputs:
        outtrace=causal version of trace (numpy 1d float array)
        timetag=number of samples that outtrace delta response is BEFORE intrace delta response
    ...
    x=np.array(intrace,dtype='double')   # Convert to double for manipulations
    ...
    if returnOrigDType:
        yout=np.array(y,dtype=data_type)   # Force back to original data type:
        return yout, timetag
    else:
        return y, timetag
